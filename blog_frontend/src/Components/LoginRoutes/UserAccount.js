...
const btnClick=async (e) => {
    ...
    const fetchFollowingUser=async () => {
        try {
            const response=await axios({
                url: "http://localhost:8000/follow/following-user-list",
                method: "GET",
                headers: {Authorization: token},
            });
    ...
    const PostFollowingUser=async () => {
        ...
        try {
            const response=await axios({
                url: `http://localhost:8000/blog/get-user-blogs?userId=${userId}&skip=${skip}`,
                method: "GET",
                headers: {Authorization: token},
            });
                                                                        ...
                                                                        return (<div className="blogList" key={blog._id} id={blog._id}>
                                                                                    <div className="user-fstln">
                                                                                        <div className="user-pic">
                                                                                            <Avatar src={user.image  && <img src={`http://localhost:8000/${user.image?.path}`} />} />
                                                                                            <div>

    ...
    const logoutFun=async (e) => {
        ...
        try {
            const response=await axios({
                url: "http://localhost:8000/auth/logout-from-all-device",
                method: "POST",
...
// Fetch My Blogs
    const fetchMyBlogsData=async () => {
        ...
        try {
            const response=await axios({
                url: `http://localhost:8000/blog/get-my-blogs?skip=${skip}`,
                method: "GET",
                headers: {Authorization: token},
            });
...
// Fetch not following users list    
    useEffect(() => {
        const fetchNotFollowingUsers=async () => {
            ...
            try {
                const response=await axios({
                    url: "http://localhost:8000/follow/notfollowing-user-list",
                    method: "GET",
                    headers: {Authorization: token},
                })
...
// Follow - Unfollow
    const btnClick=async (e) => {
        ...
        try {
            const response=await axios({
                url: "http://localhost:8000/follow/follow-user",
                method: "POST",
...
//Delete Blog
    const deleteBlogAPIFun=async (e) => {
        ...
        try {
            const response=await axios({
                url: "http://localhost:8000/blog/delete-blog",
                method: "POST",
                headers: {Authorization: token},
                data,
            })
    ...
    return (<> 
            <div id="profile" className={loginData.userId}>
                <div id="profile-card">
                    ...
                    <div id="end">
                        <div id="image-area"><img src={`http://localhost:8000/${loginData.userimage.path}`} alt={loginData.userimage.name} /></div>
                        <div id="left">
                            <div id="intro">
                ...
                <div id="suggestion">
                    {
                        notFollowing.map(user => {
                            return (<div className="sug-box" key={user._id} id={user._id}>
                                        <div id="image-area"><img src={`http://localhost:8000/${user.image?.path}`} /></div>
                            ...
                            return (<div className="blogList" key={blog._id} id={blog._id}>
                                        <div className="user-fstln">
                                            <div className="user-pic">
                                                <Avatar  src={loginData.userimage  && <img src={`http://localhost:8000/${loginData.userimage.path}`} />} />
                                                <div>
                                        ...
                                        <div className="blog-cnt">
                                            ...
                                            <div className="blog-img">
                                                {blog.image && <img src={`http://localhost:8000/${blog.image?.path}`} />}
                                            </div>
                                        </div>
                ...
                {followerPop && <div id="follower" className="follow-content">
                                    <div className="follow-box" style={colors[theme]}>
                                        ...
                                        <Follow endpointA={"http://localhost:8000/follow/follower-user-list"} endpointB={"http://localhost:8000/follow/following-user-list"} button={"Follower"} />
                                    </div>
                                </div>}
                {followingPop &&    <div id="following" className="follow-content">
                                        <div className="follow-box" style={colors[theme]}>
                                            ...
                                            <Follow endpointA={"http://localhost:8000/follow/following-user-list"} button={"Following"} />
                                        </div>
                                    </div>}

...
export function Home() {
    ...
    const {isLogin,loginData}=useContext(details);  //isLogin=true; 
    ...
    const fetchSearchUsers=async (e) => {
        ...
        try {
            const response=await axios({
                url: `http://localhost:8000/auth/searchusers?search=${search}`,
                method: "GET",
            })
...
//Theme change toggle
    ...
    <>
        <div id="home">
            <nav>
                <img src="/nexuslogo.jpg" onClick={() => navigate("/dashboard")} />
                <div id="item-box">
                   { isLogin ? (
                                <div id="login-nav">
